...
describe('AuthService', () => {
    ...
    describe('validateUser', () => {
        ...
        it('should throw UnauthorizedException if user is not found', async () => {
            jest.spyOn(usersService, 'findOne').mockImplementation(async () => { throw new NotFoundException(`User not found!`) });

            try {
                const result = await service.validateUser('nonexistentuser', 'testpassword');
            } catch (err) {
        ...
        it('should return null if password is incorrect', async () => {
            ...
            const result = await service.validateUser(mockUser.username, 'testpassword');
    ...
    describe('profile', () => {
        it('should call usersService.findOne with the correct argument', async () => {
            jest.spyOn(usersService, 'findOne').mockImplementation(async () => (mockUser));

            const result = await service.profile(mockUser.username);

            expect(usersService.findOne).toHaveBeenCalledWith({ username: mockUser.username });
            expect(result).toEqual(mockUser);
        });
    });
